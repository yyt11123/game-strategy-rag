...
/**
 * 查文档工具：把 RAG 检索封装成一个 Tool，让 Agent 可以自主决定"要不要查文档"。
 *
 * 在普通 RAG 模式下，用户提问一定走检索流程——这是硬编码的。
 * 但在 Agent 模式下，大模型会先分析用户问题，自己判断：
 * - "这个问题文档里可能有答案" → 调用本工具（查文档）
 * - "这是个纯计算问题" → 调用 CalculatorTool（计算）
 * - "既要查文档又要算" → 先调本工具，再调 CalculatorTool
 *
...
public class StrategyTool {
     ...
     * 调用此工具从知识库中搜索相关的内容。
     *
     * @param query 用户想查询的问题，最好提炼成关键词形式
     * @return 从文档中检索到的相关内容（包含来源文件名）
     */
    @Tool("查询文档资料库，搜索上传或预设文件中的相关内容")
    public String searchStrategy(
            @P("要查询的问题或关键词，如'这份文件讲了什么'、'总结文档要点'")
            String query
    ) {
        // 在知识库中检索相关片段
